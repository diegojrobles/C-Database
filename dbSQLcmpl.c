...
const uint16_t TABLE_MAX_ROWS = ROWS_PER_PAGE * TABLE_MAX_PAGES;
...
void read_input(InputBuffer* input_buffer) {
    ...
    if (!fgets(input_buffer->buffer, input_buffer->buffer_length, stdin)) {
...
void* row_slot(Table* table, uint32_t row_num){
    ...
    if(page == NULL){
        // Allocate Memory only when page is accessed
...
//Handles meta-commands (starting with '.'). Currently only supports ".exit".
